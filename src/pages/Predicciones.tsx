...
import { ShieldCheck, BarChart3, PieChart as PieIcon, ScatterChart as ScatterIcon, CheckCircle2, Loader2, AlertTriangle } from 'lucide-react';
...
export const Predicciones: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || "No se pudo conectar con el Motor IA");
    } finally { 
      setLoading(false); 
    }
  };
    ...
    <div className="h-[90vh] flex flex-col space-y-3 bg-slate-50 p-4 overflow-hidden">
      ...
      <div className="flex justify-between items-center bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="bg-slate-950 text-white px-4 py-1.5 rounded-lg font-bold text-[10px] flex items-center gap-2 hover:bg-slate-800 disabled:opacity-50 transition-colors"
          >
      ...
      {errorMsg && (
        <div className="bg-rose-50 border border-rose-200 p-4 rounded-xl flex items-center gap-3 text-rose-700">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-widest">Error del Modelo</p>
      ...
      {data && !errorMsg && (
        <div className="flex-1 flex flex-col gap-4 min-h-0 animate-in fade-in duration-500">
          ...
          {/* Fila Superior: Tabla Dinámica Completa + Dona */}
          <div className="flex gap-4 h-[40%] min-h-0">
            
            {/* Tabla Dinámica (Lee directamente las columnas del modelo) */}
            <div className="flex-[7] bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
              <div className="px-4 py-2 border-b border-slate-100 bg-slate-50/50 text-[9px] font-black uppercase text-slate-400 tracking-widest shrink-0">
                Top Riesgo Crítico (Predicción IA)
              </div>
              <div className="flex-1 overflow-y-auto">
                <table className="w-full text-left text-[11px] whitespace-nowrap">
                  <thead className="sticky top-0 bg-white shadow-sm z-10">
                    <tr className="text-slate-400 font-bold border-b border-slate-100">
                      {/* Generar los Headers automáticamente leyendo las llaves del JSON */}
                      {data.table_data && data.table_data.length > 0 && 
                        Object.keys(data.table_data[0]).map((colKey) => (
                          <th key={colKey} className="px-4 py-2 uppercase tracking-wider">
                            {colKey.replace(/_/g, ' ')} {/* Quitamos los guiones bajos para que se vea limpio */}
                          </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {data.table_data?.map((row: any, i: number) => (
                      <tr key={i} className="hover:bg-indigo-50/30 transition-colors">
                        {/* Generar las Celdas automáticamente */}
                        {Object.keys(row).map((colKey) => (
                          <td key={colKey} className="px-4 py-2 text-slate-600 font-medium">
                            {/* Le damos un color especial si es la columna de churn o el ID para que no se vea aburrido */}
                            {colKey === 'prob_churn' ? (
                              <span className="text-rose-600 font-black">{row[colKey]}%</span>
                            ) : colKey === 'user_id' ? (
                              <span className="text-indigo-600 font-bold font-mono">{row[colKey]}</span>
                            ) : (
                              row[colKey]
                            )}
                          </td>
            ...
            {/* Gráfica de Dona (Riesgo Global) */}
            <div className="flex-[3] bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center relative shrink-0">
              ...
              <ResponsiveContainer width="100%" height={120} minWidth={1} minHeight={1}>
                <PieChart>
                  <Pie 
                    data={[{value: data.summary?.promedio_churn || 0}, {value: 100 - (data.summary?.promedio_churn || 0)}]} 
                    innerRadius={35} 
                    outerRadius={48} 
                    paddingAngle={4} 
                    dataKey="value" 
              ...
              <div className="text-center mt-[-30px]">
                <p className="text-3xl font-black text-slate-900 leading-none">
                  {data.summary?.promedio_churn?.toFixed(1) || '0.0'}%
                </p>
          ...
          {/* Fila Inferior: Ciudades + Scatter de Ingreso vs Churn */}
          <div className="flex gap-4 h-[50%] min-h-0">
            <div className="flex-1 bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col">
              <h3 className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest flex items-center gap-1">
                <BarChart3 size={12} className="text-indigo-600" /> Hotspots de Riesgo por Ciudad
              </h3>
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                <BarChart data={data.top_cities} layout="vertical" margin={{left: -20}}>
                  <XAxis type="number" hide />
                  <YAxis dataKey="ciudad" type="category" width={90} style={{fontSize: '9px', fontWeight: 'bold', fill: '#64748b'}} tickLine={false} axisLine={false} />
            ...
            <div className="flex-1 bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col">
              <h3 className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest flex items-center gap-1">
                <ScatterIcon size={12} className="text-rose-500" /> Probabilidad de Churn vs Ingreso Mensual
              </h3>
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
